...
class CrawlSchedulerDaemon:
    ...
    async def _run_cycle(self):
        ...
        for src in sources:
            ...
            self.current_running_seed_name = name
            try:
                ...
                self.current_running_seed_name = None
    ...
    async def _filter_new_urls(self, urls: List[str]) -> List[str]:
        if not urls:
            return []
        async with self.session_factory() as session:
            # PostgreSQL ANY() 쿼리로 한 번에 기존 URL 조회
            stmt = text("SELECT url FROM articles WHERE url = ANY(:urls)")
            res = await session.execute(stmt, {"urls": urls})
